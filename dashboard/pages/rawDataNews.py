...
dash.register_page(__name__, path='/rawNewsData')
...
layout =     html.Div([
                ...
                dbc.Row([dbc.Col(html.Div(html.Div(id="raw_data_news")))], style=BOX_STYLE),
...
# Callback 1
@callback(
    Output(component_id='raw_data_news', component_property='children'),

    Input(component_id='dummy-input', component_property='value')


)
def update_rawData(dummy_input):
    return getgraphs.GetRawData(df_news) #html.Div(dash_table.DataTable(df_news.to_dict('records'), [{"name": i, "id": i} for i in df_news.columns]))
